...
通常待機中の短押しは横位置を3分割します。左・右は頭部の単発接触と同じHappy反応を出しながら、1回につき押した方向へ7°進みます。反対側なら7°戻し、目標角を±45°で飽和させます。最後の操作から余韻を置いて低速で正面0°へ戻します。画面由来のHappy反応中も横サーボ停止後なら次の左右入力を受け付けます。中央は同じHappy反応だけに接続し、サーボを動かしません。発話中、実際のなでなで中、較正中、起動ガイド中、ほかのサーボ動作中は新しい横向きを受け付けません。横移動中と完了後650 msは機械音を声に数えないようマイク判定を抑制します。
