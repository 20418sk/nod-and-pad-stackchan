...
較正後は`ALL TESTS COMPLETE!`で停止し、画面タップごとに`YOUR PRIVACY`（NO CAMERA / NO RECORDING / NO WORD ANALYSIS / LOCAL PROCESSING ONLY）、`READY TO LISTEN`を表示します。`NO WORD ANALYSIS`は、音量とタイミングの信号処理は行う一方、言葉の認識や意味解析は行わないという意味です。最後の`TAP TO START`までは発話判定・方向推定・ノイズ学習を止め、説明中の声やタップ音を誤って反応へ使いません。カメラを初期化しないこともプライバシー設計の明示的な一項目です。画面上の文章とエラー名は英語へ統一しています。失敗時は追加動作を止め、トルク解放を要求し、エラー顔と赤LEDを表示します。BSPの自動トルク解放も明示的に有効化します。
