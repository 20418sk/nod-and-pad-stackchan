...
声を聞いている間は小さくうなずき、話し終わると回数・深さ・速さを少しずつ変えたうなずきで応えます。頭をなでたり軽く触れたりすると、公式StackChan由来のHappy表情、ハート、照れ頬で喜びます。
...
| 発話中 | 緑LED。約1.6秒間隔で20° → 16.8° → 20°の浅いうなずき |
...
発話開始には120 msの継続、終了には600 msの沈黙が必要です。発話終了時のうなずきは、浅め11～13°が35%、中間8～10°が45%、深め5～7°が20%です。深めは安全のため必ず1回に限定します。ほかは約30%が2回です。下降速度185～215、復帰速度165～190、保持時間も制限範囲内で変え、直前と完全に同じ計画を避けます。

デュアルマイクの左右推定は、短いタップで表示できる`DIR:L/C/R/?`という診断値だけに使用します。室内反射などで揺れるため、横サーボへは接続していません。
...
- 画面を短くタップ: デバッグ表示のON/OFF
- 画面を約1.2秒長押し: 約3秒のマイク再較正
...
本体上の文章はコンテスト展示を考慮して英語へ統一しています。起動時は`SERVO TESTING`から始まり、完了後の`SERVO TEST COMPLETE!`でタップするまで停止します。タップすると`MIC CALIBRATION`、`PLEASE BE QUIET`、`3..`→`3..2..`→`3..2..1..`と表示しながら環境音を較正します。その後`ALL TESTS COMPLETE!`→`YOUR PRIVACY`（録音なし・音声内容の解析なし・本体内処理のみ）→`READY TO LISTEN`を1画面ずつタップで読み進めます。後半2画面はプライバシー説明と開始確認です。最後の`TAP TO START`で通常の顔へ入ります。説明中は声への反応とノイズ学習を始めません。画面長押し時も同じ形式で再較正します。
